					   ...
					   const RandomEngineAndDistribution & random)
{
  ...
  //
    ...
    // Fraction of muon's energy transferred to the photon
    ...
    double xmax = 1.;
    int npar = 3 ; //Number of parameters
				  ...
				  const RandomEngineAndDistribution & random) const 
{
    ...
    const double theta = gbteth(particle.momentum().E(),fastsim::Constants::muMass,xp,random)*fastsim::Constants::muMass/particle.momentum().E();
				    ...
				    const RandomEngineAndDistribution & random) const 
{
    ...
    int Z = 14; // silicon
    double d = 0.13*(0.8+1.3/Z)*(100.0+(1.0/ener))*(1.0+efrac);
