	...
	*/
    class Geometry
    {
		...
		*/
		...
		std::vector<std::unique_ptr<BarrelSimplifiedGeometry>> barrelLayers_;   //!< The vector of barrel layers (increasing radius)
		...
		const bool useTrackerRecoGeometryRecord_;  //!< Use GeometricSearchTracker (active layers/reco geometry). Can be used to get position/radius of tracker material that reflects active layers
		const std::string trackerAlignmentLabel_;  //!< The tracker alignment label
		const std::vector<edm::ParameterSet> barrelLayerCfg_;  //!< The config in which all parameters of the barrel layers are defined
